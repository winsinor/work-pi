...
Enable raw event debug logging: `cfg["touch"]["debug"] = true` in `config.json`.
...
- Tap anywhere to dismiss; tap bottom 32% to power off
- `POWEROFF_Y_FRAC = 0.68` in `stats.py`
- **Do not use `anchor="rt"` with bitmap fonts** — it's silently ignored; use explicit `x = W - VAL_W + 2`
...
- `touch.py` has unconditional `print("[touch] press")`, `print("[touch] drop: ...")`, `print("[touch] tap ...")` statements that always log. These should be gated on the `debug` flag or removed to reduce journal noise.
- Touch calibration defaults (`min_x=200, max_x=3900, min_y=200, max_y=3900`) may not be perfectly aligned for this screen — adjust via `config.json` `touch` section if taps register in wrong position.

## Branch

Active development branch: `claude/fix-display-issues-ZghAt`
